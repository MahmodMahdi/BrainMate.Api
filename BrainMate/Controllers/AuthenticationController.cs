...
namespace BrainMate.Api.Controllers
{
	...
	public class AuthenticationController : AppControllerBase
	{
		...
		[SwaggerOperation(Summary = "إرسال كود تأكيد الباسورد للمرافق", OperationId = "CaregiverSendResetPasswordCode")]
		[HttpPost(Routing.AuthenticationRouting.CaregiverSendResetPassword)]
		public async Task<IActionResult> CaregiverSignIn([FromForm] CaregiverSendResetPasswordCommand command)
		{
			var response = NewResult(await _mediator.Send(command));
			return response;
		}
		...
		[Authorize]
		[SwaggerOperation(Summary = "آخر إذا كان منتهي Token وإعطاء Token التحقق من", OperationId = "RefreshToken")]
		...
		[Authorize]
		[SwaggerOperation(Summary = "Token التحقق من صلاحية", OperationId = "ValidateToken")]
