...
namespace BrainMate.Core.Features.Authentication.Commands.Handler
{
                                        ...
                                        IRequestHandler<ResetPasswordCommand, Response<string>>
    {
        ...
        public async Task<Response<JwtAuthenticationResponse>> Handle(SignInCommand request, CancellationToken cancellationToken)
        {
            ...
            // check if Patient lock the caregiver accoun
            if (user!.LockoutEnd > DateTime.Now)
            {
                return BadRequest<JwtAuthenticationResponse>(_stringLocalizer[SharedResourcesKeys.YourAccountIsLockedByPatient]);
            }
        ...
        public async Task<Response<JwtAuthenticationResponse>> Handle(CaregiverSignInCommand request, CancellationToken cancellationToken)
        {
            // Check if user is exist or not 
            var user = await _userManager.FindByEmailAsync(request.Email!);
